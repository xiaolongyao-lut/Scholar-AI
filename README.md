...
- 检查 OCR 配置和扫描型材料处理路径。
- 通过 [MCP 工具箱](docs/claude-codex-toolbox.md) 让 Claude / Codex 调用本地文献能力，但不暴露原始 API key。
...
[`agent_mcp_server/`](agent_mcp_server/README.md) 提供本地 MCP server，让 Claude、Codex 或其他 MCP 客户端调用 Scholar AI 的文献检索、证据包、OCR、写作导出、Agent Workspace 和安全源码查看能力。
...
桌面端负责文献库、PDF 阅读、模型配置、凭证管理和审计查看。MCP 工具箱负责把这些本地能力暴露给用户授权的 AI 客户端：`source.*` 不依赖后端，只读白名单源码；`literature.*` 走后端 HTTP API；`workflow.*` 和 `artifact.*` 负责可复验的研究动作与产物。工具结果在返回 MCP 客户端前会统一脱敏、限长，并保留可机器读取的 refs / locator / integrity 状态。

## RAG 与证据架构

Scholar AI 的核心是一条本地文献 RAG 与证据链路。它不是单次向量检索，而是把本地材料变成可定位 chunk，再按不同场景进入稳定 refs、智能研读上下文、Wiki 联合召回和证据完整性检查。详细模块、代码入口和降级边界见 [RAG 与证据架构](docs/rag-evidence-architecture.md)。
...
这条链路让 Scholar AI 能把“检索到了什么”变成“哪个项目、哪篇材料、哪个 chunk、哪个 locator 和哪个完整性状态支持了这句话”。不同入口不会假装都走同一条重链路：`search_refs` 提供稳定、只读的 refs；智能研读会按开关组合 TOLF、RRF、结构化邻居和混合检索；证据包会把项目 chunk、Wiki refs 和知识 refs 收束成可复查结果。外部 Claude / Codex 客户端通过 MCP 读取的是这些受控工具结果。
