...
本仓库包含桌面端、FastAPI 后端、RAG / evidence pipeline、MCP 工具箱、前端界面、测试和发布脚本。适合从源码运行、检查架构、复用本地科研流程，或在现有工具链上继续开发。

发布包和历史版本见 [GitHub Releases](https://github.com/xiaolongyao-lut/Scholar-AI/releases)。
...
[`agent_mcp_server/`](agent_mcp_server/README.md) 提供本地 MCP server，面向 Claude / Codex 暴露 Scholar AI 的受控工具：`source.*`、`literature.*`、`workflow.*` 和 `artifact.*`。它通过本地 token 调用后端，不接收原始 provider key。
...
Scholar AI 的检索链路以项目、材料、页码和 chunk 为基本定位单元。材料入库后会写入 doc / chunk / page locator 相关存储；检索、Wiki 合成、智能研读、综述写作和 MCP 工具调用会沿用这些定位信息，并在证据包阶段记录 refs、locator coverage 和完整性状态。详细模块、代码入口和降级边界见 [RAG 与证据架构](docs/rag-evidence-architecture.md)。
