...
- Phase 4 可启动：文件夹遍历模块和提取管道实现（phase-plan 中的 folder traversal + extraction pipeline）。keyword_prefilter 已经过实现 + 测试双重验证，可作为管道中的过滤阶段直接集成。
- Phase 4 注意：Zotero jasminum-outline.json 仅含 PDF 大纲，不含结构化元数据——此约束不影响 keyword_prefilter（它处理已解析的 dict 记录，不直接读取文件），但影响提取管道的元数据获取策略。