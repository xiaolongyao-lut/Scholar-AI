...
- ⛔ **所有安全自主任务已完成。** 检索管道三模块全部实现、测试（16/16）、真实数据验证、文档更新均已交付。
- 🛑 **HARD-STOP — Intelligent Chat（WAITING FOR USER）**：phase-plan 最后一个 Must Deliver——"Intelligent chat grounded in the extracted literature base"——需要 LLM 集成（新外部依赖），属于 hard-stop 类决策。Owner 需决策：
  1. LLM 框架选择（openai / langchain / litellm / 其他）
  2. API 密钥管理策略
  3. 上下文窗口预算（token 限制 vs. 文献量平衡）
  4. 对话记忆方案（单轮 vs. 多轮 session）
