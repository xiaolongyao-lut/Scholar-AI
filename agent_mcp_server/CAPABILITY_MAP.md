...
如果用户说“启动文献助手 / 打开 Scholar AI / 打开前端”，在这个同一个文献助手 MCP 内先调用：

```text
literature.launch_desktop       # 打开或附着源码桌面窗口 `文献助手`
```

如果没有已健康的桌面运行时，`literature.launch_desktop` 必须打开一个可见 PowerShell 终端，并在仓库根目录运行源码桌面命令：
