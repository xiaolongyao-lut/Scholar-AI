...
# --- Generation key-pool failover ---------------------------------------------
# Phase A2 helper: lets callers wrap their invoke() with credential rotation
# without each one re-implementing pool lookup. Returns a no-arg callable
# suitable to pass as invoke= to gated_call.

_GENERATION_CATEGORY = "generation"


def _resolve_generation_pool() -> Any:
    """Return a KeyPool with non-empty generation creds, else None.
    ...
    so unit tests that mock the HTTP layer can opt out of pool rotation.
    Never raises — pool is opt-in failover.
    """
...
def with_generation_pool_failover(
    ...
    """Wrap an invoke factory so it walks the generation pool with cooldown.

    invoke_factory(cred) -> () -> Any:
        Returns a no-arg callable that performs the actual HTTP call. When
        cred is None (no pool), use caller-supplied default api_key /
        base_url / model. When cred is a Credential, use cred.api_key,
        cred.base_url, cred.model.

    Returns a no-arg invoke suitable for gated_call(invoke=...).

    Usage::

        invoke=with_generation_pool_failover(
            lambda cred: lambda: _call_ark_once(
                prompt,
                cred.api_key if cred else default_api_key,
                model=cred.model if cred else default_model,
                base_url=cred.base_url if cred else default_base_url,
            )
        )
    ...
    per ``tests/test_generation_dispatch_parity.py``: attempt order,
    cooldown state, return value, and (last) exception are preserved.

    Set ``LIT_DISPATCHER_FOR_GENERATION_WRAPPER=0`` in the environment to
    force the legacy ``pool.try_call`` path for emergency rollback. No
    code change required to revert.
    """
    pool = _resolve_generation_pool()
    if pool is None:
        return invoke_factory(None)

    use_dispatcher = os.environ.get("LIT_DISPATCHER_FOR_GENERATION_WRAPPER", "1") == "1"

    if not use_dispatcher:
        # Legacy path retained behind a flag for emergency rollback.
        def _walk_legacy() -> Any:
            return pool.try_call(_GENERATION_CATEGORY, lambda cred: invoke_factory(cred)())
        return _walk_legacy

    # Default path: dispatcher + adapter. See §13.1c, §13.1f for rationale.
    def _walk_via_dispatcher() -> Any:
        ...
        if not candidates:
            # Pool ran out / category empty — match legacy "no creds" path
            # by falling back to caller-supplied defaults via None.
            return invoke_factory(None)()

        # Capture the actual underlying exception so we can re-raise it with
        # its original class on all-fail (preserves caller ``isinstance``
        # checks). Box is appended-to as candidates are tried.
        last_exc_box: list[BaseException] = []

        def _fn(cred: Any) -> Any:
            try:
                return invoke_factory(cred)()
        ...
        #   - if any real provider exception occurred, re-raise the LAST one
        #     with its original class
        #   - otherwise (only cooled-down skips), raise the same RuntimeError
        #     that pool.try_call would on "no credentials available"
        if last_exc_box:
