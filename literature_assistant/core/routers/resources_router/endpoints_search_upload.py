...
# ---------------------------------------------------------------------- #
# A16b: reparse a project's PDFs with the active backend (marker)
# ---------------------------------------------------------------------- #


@_rr.router.post("/projects/{project_id}/reparse-with-marker")
async def reparse_project_with_marker(project_id: str) -> dict[str, Any]:
    """Re-extract every PDF material in a project using the active PDF backend.
    ...
    """
    from pdf_backends import get_pdf_backend  # local — avoid cycles at module load
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail="Writing resource store unavailable") from exc

    try:
        materials = list(store.list_materials(project_id))
    except Exception as exc:  # noqa: BLE001
    ...
    for material in materials:
        material_id = getattr(material, "material_id", "") or ""
        ...
        if source_path.suffix.lower() != ".pdf":
            ...
            continue
        try:
            payload = _rr._extract_document_payload_from_path(source_path.name, source_path)
        ...
        except Exception as exc:  # noqa: BLE001 — surface per-material failure to client
            failed.append(
                {
                    "material_id": material_id,
                    "title": title,
