...
class ChunkSearchRefMetadataPayload(BaseModel):
    ...
    Args:
        ...
        figure_candidate_detail: Bounded figure/table metadata that lets UI and
            MCP clients inspect real chunk image evidence without reading the
            whole chunk body.
...
class ChunkSearchRefPayload(BaseModel):
    """Small search hit returned to MCP clients without the full source body.
...
class FigureTableCandidatePayload(PdfAnchorFields):
