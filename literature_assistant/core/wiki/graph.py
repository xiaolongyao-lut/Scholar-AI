...
class WikiGraphStore:
    ...
    def save(self, snapshot: WikiGraphSnapshot) -> None:
        ...
        self.save_json(snapshot)
        self.save_sqlite(snapshot)

    def save_json(self, snapshot: WikiGraphSnapshot) -> None:
        """Persist a graph snapshot to JSON for legacy callers."""
    ...
    def load_json(self) -> WikiGraphSnapshot | None:
        """Load a JSON graph snapshot, returning None when it is absent."""
    ...
    def save_sqlite(self, snapshot: WikiGraphSnapshot) -> None:
        """Persist a graph snapshot to SQLite for legacy callers."""

        if not isinstance(snapshot, WikiGraphSnapshot):
            raise TypeError("snapshot must be a WikiGraphSnapshot")
        self._write_sqlite(snapshot)

    def load_sqlite(self) -> WikiGraphSnapshot | None:
        """Load a graph snapshot from SQLite, returning None when it is absent."""
        ...
        with sqlite3.connect(str(self.sqlite_path)) as conn:
            ...
            metadata = {
                row["key"]: row["value"]
                for row in conn.execute("SELECT key, value FROM wiki_graph_metadata")
            }
            node_rows = conn.execute(
                """
                SELECT node_id, page_path, kind, title, status, content_hash,
                       frontmatter_id, metadata_json
                FROM wiki_graph_nodes
                ORDER BY node_id
                """
            ).fetchall()
            edge_rows = conn.execute(
                """
                SELECT edge_id, source_id, target_id, edge_type, weight, confidence,
            ...
            WikiGraphNode.from_dict(
                {
                    ...
                    "frontmatter_id": row["frontmatter_id"],
...
def compute_blast_radius(
    ...
    *,
    ...
    best_seen_relevance: dict[str, float] = {seed: 1.0}
    ...
    while queue:
        ...
        for edge, next_node in adjacency.get(node_id, ()):
            ...
            if next_relevance < min_relevance:
                continue
            if best_seen_relevance.get(next_node, -1.0) >= next_relevance:
                continue
            best_seen_relevance[next_node] = next_relevance
            queue.append(
                (
                    next_node,
                    next_relevance,
