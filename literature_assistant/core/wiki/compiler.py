...
class WikiCompiler:
    ...
    def compile_paper_with_llm(
        ...
        *,
        ...
        _identity = render_identity_header("wiki_paper_summary")
        if _identity:
            prompt = f"{_identity}\n\n{prompt}"
    ...
    def compile_concepts_with_llm(
        ...
        *,
        ...
        _identity = render_identity_header("wiki_concept_extract")
        if _identity:
            prompt = f"{_identity}\n\n{prompt}"
        llm_response = self.llm_gateway.generate(LLMRequest(prompt=prompt))
        parsed, parse_error = validate_json_response(llm_response.text)
    ...
    def compile_claims_with_llm(
        ...
        *,
        ...
        prompt = template.replace("{source_text}", source_text)
        _identity = render_identity_header("wiki_claim_extract")
        if _identity:
            prompt = f"{_identity}\n\n{prompt}"
        llm_response = self.llm_gateway.generate(LLMRequest(prompt=prompt))
        parsed, parse_error = validate_json_response(llm_response.text)
    ...
    def compile_synthesis_with_llm(
        ...
        *,
        ...
        _identity = render_identity_header("wiki_synthesis")
        if _identity:
            prompt = f"{_identity}\n\n{prompt}"
        llm_response = self.llm_gateway.generate(LLMRequest(prompt=prompt))
        parsed, parse_error = validate_json_response(llm_response.text)
