...
def load_all_chunks(chunk_store_dir: Path) -> dict[str, list[dict]]:
    ...
    支持两种格式：
    - v2: {project_id}/manifest.json + {project_id}/*.jsonl
    - v1: {project_id}_chunks.json (legacy)
    """
    materials: dict[str, list[dict]] = {}
    
    # Load v2 format (subdirectories with manifest.json)
    for project_dir in chunk_store_dir.iterdir():
        ...
        try:
            ...
            for material_id, entry in manifest.get("materials", {}).items():
                ...
                if not relative_path:
                    continue
                if "_quarantine" in str(relative_path):
                    continue
                jsonl_path = project_dir / relative_path
    ...
    # Fallback: Load v1 format (legacy *.json)
    if not materials:
