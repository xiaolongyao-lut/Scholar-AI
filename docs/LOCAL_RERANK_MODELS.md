# 本地 Rerank 回退模型 — 切换其他开源模型

文献助手主路径走云端 rerank API(SiliconFlow / DashScope / 等 Cohere 兼容服务)。当 API 失败、被限流、或网络断开时,系统会自动回退到一个**本地 cross-encoder 模型**对候选文献重新排序;如果连本地模型也不可用,最终退到 `hybrid_score` 静态排序兜底。
...
| **网络不稳/内网** | 把权重打包进 onedir 安装包(或私有 HF mirror) |
