...
async def _fake_rerank(query, candidates, ...):
    assert query == "海洋碳循环"
    assert {item["chunk_id"] for item in candidates} == {"cn1", "en1"}
```
...
Recall@10 dropped slightly (0.57 -> 0.50), which was considered an acceptable tradeoff for that experiment.
Tests: 12/12 passed.
