...
| J11 | Skill 导入/导出 | Skill 包导入导出 | Settings → Skill 导入/导出按钮 | `/api/skill/import` `/api/skill/export` | skill-import-export | 是/P1 应做 | Claude(后端/接口) | ⚠️ 部分实现：`POST /skills/import` 已完成；export 端点缺失（19 个端点中无 export） |
