...
| G5 | Wave 4 搜索 | Wiki 全文搜索 | Wiki 页显示搜索框 | `/api/wiki/search` | llm-wiki Wave 4 | 是/P0 必做 | Claude(后端/接口) | ✅ 已完成：`POST /api/wiki/search` 新增为 canonical 端点；`POST /api/wiki/query` 保留为 legacy 别名；两者共享 `_wiki_search_impl` 实现；向后兼容；前端搜索框待 Codex 实现 |
