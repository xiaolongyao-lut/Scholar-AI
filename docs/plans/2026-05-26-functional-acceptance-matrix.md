...
| B5 | 成本档位统一 | low/medium/high/xhigh/max 五档 | Settings → API 凭证 → 默认档位 | `CredentialStrategyHint` enum 扩展 | settings-smartread-ui-followup tier slice | 是/P0 必做 | Claude | ✅ 已完成 (2026-05-26 commit b5b0ca3e)：`CredentialStrategyHint` 新增 LOW/MEDIUM/HIGH/XHIGH/MAX canonical 枚举 + DEFAULT/CHEAP/FAST/QUALITY legacy 枚举；`normalize_strategy_hint()` 映射 cheap→low, default→medium, fast→medium, quality→high, 未知→medium；11 测试覆盖 canonical/legacy/surface/defaults/case/whitespace (`tests/test_credentials_strategy_hint_mapping.py`)；代码：`literature_assistant/core/models/credentials.py:39-68` enum, `:173-217` normalize；验证：`pytest tests/test_credentials_strategy_hint_mapping.py -v` 11/11 passed |
...
| ID | 功能 | 期望效果 | 前端验收点 (界面是否齐) | 后端验收点 (链路是否通) | 来源 plan | 文献助手必要性/优先级 | 执行方 | 状态 |
|----|------|----------|------------------------|------------------------|-----------|----------------------|--------|------|
| D1 | Runtime 凭证 Phase 6 | 讨论时动态选择 API 凭证 | Discussion 面板显示凭证选择器 | `runtime_credentials_store` 表 | runtime-credentials Phase 6 | 是/P0 必做 | Claude→Codex | ⚠️ 部分实现：模型/后端支持 `credential_id`；Settings 讨论 profile 有凭证选择，Discussion 面板运行时选择仍需核验 |
| D2 | Runtime 凭证 Phase 7 | 凭证采样端点 `/api/credentials/sample` | 无前端 UI (后端 API) | `/api/credentials/sample` 200 OK | runtime-credentials Phase 7 | 是/P1 应做 | Claude | ✅ 已完成 (2026-05-26 commit b5b0ca3e)：`POST /api/credentials/sample?category={generation|embedding|rerank}&strategy_hint={tier}` 按 category + strategy_hint 选择凭证；精确匹配 strategy_hint 优先，回退到最高 priority (数值越小优先级越高)；返回 `RuntimeCredentialPublic` (masked api_key)；8 测试覆盖精确匹配/legacy 映射/priority 回退/category 过滤/无可用凭证 404/默认 category=generation/默认 strategy_hint=medium/api_key 掩码 (`tests/test_credentials_sampling.py`)；代码：`literature_assistant/core/routers/credentials_router.py:208-254` sample_credential endpoint；验证：`pytest tests/test_credentials_sampling.py -v` 8/8 passed；依赖注入修复：router 所有端点使用 `Depends(get_credential_store)` 支持测试隔离 |
...
| I5 | Credentials 采样 B8 | 采样端点 `/api/credentials/sample` | 无前端 UI (后端 API) | `/api/credentials/sample` 200 OK | 0182-release B8 | 是/P0 必做 | Claude(后端/接口) | ❓ 未定位：实际采样是 `/sampling` |
