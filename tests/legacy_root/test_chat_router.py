...
def test_resolve_request_llm_config_uses_chat_defaults_when_sampling_missing() -> None:
    llm = LLMConfig(
        provider="DeepSeek",
        api_key="test-key",
        model="deepseek-chat",
    ...
    resolved = _resolve_request_llm_config(llm, task="chat", sampling=None)
...
def test_resolve_request_llm_config_applies_sampling_override() -> None:
    llm = _make_llm("DeepSeek", "https://api.deepseek.com")

    resolved = _resolve_request_llm_config(
        llm,
        task="chat",
        sampling={"temperature": 0.2, "top_p": 0.6, "max_tokens": 1024},
    )

    assert resolved.temperature == 0.2
    assert resolved.top_p == 0.6
    assert resolved.max_tokens == 1024


def test_resolve_request_llm_config_merges_saved_sampling_with_request_override(monkeypatch) -> None:
    llm = _make_llm("DeepSeek", "https://api.deepseek.com")
    monkeypatch.setattr(
        "routers.chat_router.load_user_sampling",
        lambda: {"chat": {"temperature": 0.4, "top_p": 0.55, "top_k": 60}},
    )
    ...
    assert resolved.temperature == 0.2
    assert resolved.top_p == 0.55
    assert resolved.top_k == 60
    assert resolved.max_tokens == 1024
