...
class TestMAQWeightEngine:
    ...
    def test_run_small_corpus(self):
        ...
        inside, _weights, _w_dict = engine.run(queries, corpus)
...
class TestTOLFEngine:
    ...
    def test_convex_hull_reduces_noise(self):
        ...
        query_emb = rng.standard_normal((4, 32), dtype=np.float32)
        
        # 相关 chunks: 靠近 query embeddings
        related = query_emb[0:1] + rng.standard_normal((10, 32)).astype(np.float32) * 0.1
        # 噪声 chunks: 远离
        noise = rng.standard_normal((20, 32)).astype(np.float32) * 3.0 + 5.0
    ...
    def test_representative_rerank_is_disabled_by_default(self):
        """代表单元 rerank stage 默认关闭，不应改变结果顺序"""
        cfg = TOLFConfig(
            umap_n_components=3,
            umap_n_neighbors=2,
        ...
        chunks = make_chunks(20)
        emb = make_random_embeddings(20, dim=32)
        query_emb = make_random_embeddings(4, dim=32, seed=77)

        baseline = engine.run(
            goal="laser welding mechanism",
            chunks=[dict(chunk) for chunk in chunks],
            embeddings=emb,
            aspect_query_embeddings=query_emb,
        )
        rerank_disabled = engine.run(
            goal="laser welding mechanism",
            chunks=[dict(chunk) for chunk in chunks],
            embeddings=emb,
            aspect_query_embeddings=query_emb,
            representative_reranker=lambda _goal, fish: list(reversed(fish)),
            enable_representative_rerank=False,
        )

        assert [item.chunk_id for item in rerank_disabled] == [item.chunk_id for item in baseline]

    def test_representative_rerank_runs_after_evidence_gate(self):
        """启用后，代表单元 rerank 应只处理已过 evidence gate 的 fish"""
        cfg = TOLFConfig(
            umap_n_components=3,
            umap_n_neighbors=2,
            activation_threshold=0.1,
        ...
        chunks = make_chunks(20)
        emb = make_random_embeddings(20, dim=32)
        query_emb = make_random_embeddings(4, dim=32, seed=88)

        seen: dict[str, list[str] | int | None] = {"count": 0, "ids": None}

        def reverse_reranker(_goal: str, fish: list[FishResult]) -> list[FishResult]:
            seen["count"] = int(seen["count"]) + 1
            seen["ids"] = [item.chunk_id for item in fish]
            assert all(item.evidence_score > cfg.evidence_threshold for item in fish)
            return list(reversed(fish))
        ...
        reranked = engine.run(
            ...
            embeddings=emb,
            aspect_query_embeddings=query_emb,
            representative_reranker=reverse_reranker,
            enable_representative_rerank=True,
        )

        assert seen["count"] == 1
        assert seen["ids"] == [item.chunk_id for item in baseline]
        assert [item.chunk_id for item in reranked] == list(reversed([item.chunk_id for item in baseline]))
