...
const kindBadgeClass: Record<string, string> = {
  ...
  tool_result: "bg-sky-100 text-sky-700 border-sky-200",
  checkpoint: "bg-purple-100 text-purple-700 border-purple-200",
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            {lastRewindToast && (
              <div className="mx-4 mt-3 rounded-sm border border-emerald-200 bg-emerald-50 px-3 py-2">
                <div className="flex items-start gap-2">
                  <div className="flex-1">
                    <p className="font-label text-[11px] font-medium text-emerald-800">
                      回退完成（
                    ...
                    {typeof lastRewindToast.archivedCount === "number" && (
                      <p className="mt-1 font-body text-[10px] text-emerald-700/80">
                        已 archive {lastRewindToast.archivedCount} 条 turn（未删除，可 fork 找回）
                      </p>
                    )}
                    {lastRewindToast.snapshot && (
                      <p className="mt-1 font-body text-[10px] text-emerald-700/80 break-all">
                        快照：
                        <code className="px-1 bg-emerald-100 rounded text-[9px]">
                          {lastRewindToast.snapshot}
                        </code>
                      </p>
                    )}
                  ...
                  <button
                    ...
                    aria-label="关闭提示"
                    className="p-0.5 text-emerald-700/70 hover:text-emerald-900"
                  >
                    <X size={12} />
                  </button>
                </div>
