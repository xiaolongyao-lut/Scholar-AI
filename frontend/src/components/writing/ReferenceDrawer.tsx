...
const getEvidenceStatusTone = (status: MaterialEvidenceStatus) => {
  switch (status) {
    case 'used':
      return 'bg-emerald-100 text-emerald-700 border-emerald-200/70';
    case 'weak':
      return 'bg-amber-100 text-amber-700 border-amber-200/70';
    default:
            ...
            <>
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-sm border border-emerald-200/70 bg-emerald-50/60 px-3 py-1.5 font-label text-[10px] font-medium text-emerald-700 transition-all hover:bg-emerald-100"
                >
                  <Download size={10} />
                ...
                >
                  {copied ? <CheckCircle2 size={10} className="text-emerald-600" /> : <Copy size={10} />}
                  ...
                  <div className="space-y-2">
                    {reviewFindings.map((row, i) => (
                      <div key={`rf-${i}`} className={cn(
                        ...
                        String(row.severity || row.tone) === 'warn' ? 'border-amber-200/70 bg-amber-50/60' : 'border-emerald-200/70 bg-emerald-50/60'
                      )}>
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {material.focusPointsZh.length > 0 ? material.focusPointsZh.map((focusPoint, index) => (
                      ...
                      <span className="font-label text-[9px] font-medium px-2 py-0.5 bg-amber-50 text-amber-700 rounded-sm border border-amber-200/70">
                        缺少 focus points
                      </span>
                    )}
                  </div>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {anchor.materialId && !material ? (
                        <span className="rounded-sm border border-amber-200/70 bg-amber-50 px-2 py-0.5 font-label text-[9px] text-amber-700">
                          missing material
                        </span>
                      ) : anchor.materialId ? (
                        <span className="rounded-sm border border-emerald-200/70 bg-emerald-50 px-2 py-0.5 font-label text-[9px] text-emerald-700">
                          bound material
                        </span>
                      ) : (
                        <span className="rounded-sm border border-amber-200/70 bg-amber-50 px-2 py-0.5 font-label text-[9px] text-amber-700">
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <div className={cn(
                          ...
                          entry.tone === 'ok' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700',
                        )}>
