...
function OutlineItem({ node, onDelete }: { node: OutlineNode; onDelete?: (id: string) => void }) {
  ...
  const { t } = useI18n();
  const statusColor = { done: 'bg-emerald-400', wip: 'bg-amber-400', empty: 'bg-surface-highest' }[node.status];
    ...
    <div className="p-8 max-w-3xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          ...
          disabled={adding}
          className="flex items-center gap-1.5 px-3 py-2 bg-primary text-primary-foreground rounded-lg font-label text-xs font-medium shadow-sm hover:bg-primary/90 disabled:opacity-60 transition-all"
        >
          <Plus size={14} />
      ...
      <div className="glass-card rounded-lg p-4 space-y-0.5">
      ...
      <div className="flex items-center gap-5 mt-4 px-2">
        {[
          { label: t('writing.outline.status_done'), color: 'bg-emerald-400' },
          { label: t('writing.outline.status_wip'), color: 'bg-amber-400' },
          { label: t('writing.outline.status_empty'), color: 'bg-surface-highest' },
        ].map(l => (
          <span key={l.label} className="flex items-center gap-1.5 font-label text-[10px] text-foreground/40">
