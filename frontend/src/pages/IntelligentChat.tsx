...
import { MessageCircle, RefreshCw, Send, AlertCircle, History, X, BookOpen, Sparkles } from 'lucide-react';
...
export function IntelligentChat() {
  ...
  const inputStorageKey = `intelligent-chat-input_${activeProjectId || 'default'}`;
  ...
  useEffect(() => {
    try {
      setInputValue(localStorage.getItem(inputStorageKey) ?? '');
    } catch { setInputValue(''); }
  }, [inputStorageKey]);
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gray-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-gray-900">AI 对话</h1>
            <p className="text-xs text-gray-500">文献模式：基于项目文献检索回答 · 直调模式：直接调用模型</p>
            {sessionId && (
        ...
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-gray-500 max-w-md">
              {directMode
                ? '直调模式：AI 直接回答你的问题（不检索文献库）。'
                : '文献模式：AI 会从你的文献库中检索相关内容并基于证据回答。'}
            </p>
          </div>
        ) : (
      ...
      <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
        <div className="mb-3 flex items-center gap-3">
          <div className="inline-flex items-center gap-0 rounded-lg border border-gray-300 bg-white p-0.5" role="tablist" aria-label="Chat mode">
            <button
              type="button"
              role="tab"
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md transition-colors disabled:cursor-not-allowed ${
                !directMode
          ...
          <div className={`flex-1 ${directMode ? 'opacity-40 pointer-events-none' : ''}`}>
            <TierSelector
              selectedTier={selectedTier}
              onTierChange={setSelectedTier}
              disabled={isInputDisabled || directMode}
            />
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <textarea
            ...
            disabled={isInputDisabled}
            placeholder={directMode ? '直接向 AI 提问（不检索文献）…' : 'Ask a question about your literature...'}
