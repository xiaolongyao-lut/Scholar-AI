...
| C2 | Rerank 持久化磁盘缓存层（跨进程复用，预计 60-90% 调用降本） | `rerank_cache.py` + `reranker_client.py` | 🔄 本会话执行 |
| C3 | Rerank 调用成本埋点 → `output/rerank_cost.jsonl` | `reranker_client.py` | 🔄 本会话执行 |
...
**验收**：
1. 现有 109 篇语料迁移后 manifest 完整、SHA 一致；
2. 现有评测脚本无任何修改可继续运行；
...
**状态**：
- ✅ 已完成（代码落地，2026-04-21 本轮）
- 🔧 待补丁（env 契约对齐，**关闭 §1.3 前必做，不是可选尾活**）：env 名从 USD 中心调为「调用次数为主、token 为辅、USD 仅观测」
...
| `layers/ai_adapter.py` | ❌ 未接入 | 7 处硬编码 `chat.completions.create(...)`，必须改 |
| `inspiration_engine.py` | ❌ 未接入 | 调 LLM 处需走 task="inspiration" |
| `extractor_full.py` / 任何抽取链路 | ❌ 未接入 | task="extraction" |
| 前端 Sampling 面板 + `~/.literature-lab/sampling.json` | ❌ 未接入 | 见 2.1.3 |
...
**状态**：⏸️ **BLOCKED**（2026-04-23）— 无法继续：embedding 凭证失效（`EmbeddingAPIError 401`），且环境不存在非 contextual 缓存，无法独立生成 E1-E4 对比。

**Blocker**：`chunk_vector_store.build()` 因无效 API 凭证报错，plan 中的缓存隔离策略无法验证。

**Resume 前置条件**：
1. 修复或替换有效的 embedding API 凭证
2. 为 Phase 6 corpus 生成完整的非 contextual embedding 缓存
3. 重跑 E1 → E4，完成对比表格与决策报告
...
**状态**：🔄 进行中（2026-04-22，前两个 core slice 已落地；Slice 1 completion：chunk_size_guard 模块 + rerank oversize skip + eval manifest-first + 报告头 oversize_count；Slice 2 completion：embedding 入口守护 + chunk_quarantine 隔离；remaining：manifest-first real-corpus acceptance / rerank_cost 全量合规复核）
...
#### 3.5.4 给 Team 的 prompt（Slice 2 待实装）
...
**状态**：🔄 进行中（Step 1 已落地；Step 2 的 rerank 接入已落地但 canary30 对比仍被无效 embedding credentials（HTTP 401）卡住；Step 3 的 `layers/ai_adapter.py._chat` gateway 接入也已落地并通过 focused ai_adapter + gateway bundle（35 passed）；Step 4 的 `chunk_vector_store.py` embedding gateway 接入代码与 focused embedding/vector-store/gateway bundle（33 passed）也已落地，但按计划要求的旧 `corpus_embeddings*.npy` 清理/重建仍因同一 401 blocker 未执行；Step 5 的 `query_expander.py` / `contextual_chunker.py` gateway 接入已落地并通过 focused bundle（15 passed）；补丁切片已将 `main_rag_workflow.py._generate_answer(...)` 改为通过 gateway 进入 `task="generation"`，从而清除先前的 `requests.post(...)` scoped grep blocker。由于 Step 2/4 的 runtime 验收阻塞未清，§3.6 整体继续保持进行中。）
...
4. 🔄 `chunk_vector_store.py` embedding 路径接入代码已完成：远程 embedding 现逐条走 `model_call_gateway.gated_call(...)`，gateway key 绑定 `model + normalized_text + chunking_version`，并保留现有 chunk hard-limit guard 与 manifest/hash cache guard；focused bundle `tests/test_dense_rrf_retrieval.py tests/test_chunk_size_guard.py tests/test_model_call_gateway.py -q` → **33 passed**。但计划要求的旧 `corpus_embeddings*.npy` / `corpus_embeddings_contextual.npy` 清理与后续 rebuild 未执行，因为当前 embedding credentials 仍返回 HTTP 401，无法安全完成 runtime rollout；因此 Step 4 的 runtime acceptance 继续阻塞。
...
- [ ] 跑一次 109 篇评测，第二次跑同样 query 集合时 rerank 调用次数下降 ≥ 60%（缓存命中）
...
**状态**：🔄 代码切片已完成（2026-04-22）：用户消息已补齐文档级摘要提示词原文；`contextual_chunker.py` 现改为查询期只读离线产物、miss 仅记 `output/contextual_miss.jsonl` 且不再在线调 LLM；新增 `scripts/precompute_contextual_summaries.py` 通过 §3.6 gateway 预计算 `output/contextual_summaries/{project_id}/{material_id}.json`。focused bundle `tests/test_precompute_contextual.py tests/test_contextual_chunker.py -q` → **8 passed**。但 §3.7 DoD 中“109 篇一次性预计算耗时落盘”和“线上 miss 持续为空”两项仍未执行，故本节 DoD 继续保持未勾选。
...
- [ ] 一次性预计算 109 篇用时记录在 `eval_reports/`，后续查询期 contextual LLM 调用 = 0
- [ ] `contextual_miss.jsonl` 在线运行期持续保持空（除了刚入库新 material）

---

### 3.8 Generation 打包预算 + per-paper 上限（**真省钱项 #4**）

**状态**：✅ 代码切片已完成（2026-04-22），并于 2026-04-23 完成最终回答提示词对齐：新增 `evidence_packer.py`，`main_rag_workflow.py` 现按既有 score 顺序执行同 material `jaccard>0.9` 硬去重、超 soft budget 时 `jaccard>0.7` 次优冗余裁剪、`EVIDENCE_MAX_PER_MATERIAL=2` 上限、`EVIDENCE_PACK_TOP_K=5` 默认截断，以及 `EVIDENCE_TOKEN_HARD_CAP=5000` 的低分尾部裁剪；生成提示词现与用户提供的统一规范对齐，要求 **JSON-only** 输出、固定 schema（`conclusion/evidence/limitations/next_search/status`）、真实 `[chunk_id]` 原子引用、缺失信息写“文中未提及”、冲突时 `status="conflict"`。focused bundle `tests/test_evidence_packer.py tests/test_main_rag_workflow_generation.py -q` → **10 passed**。DoD 审计完成：`scripts/audit_3_8_dod.py` 验证所有三项均通过（prompt=876 tokens ≤ budget (4000), max_per_material=2 合规, [chunk_id] 引用强制要求存在）。结果落地于 `output/audit_3_8_dod_results.json`。
...
| L8 | 没有 `requirements-pin.txt` | P2-a | (新建) | `pip freeze > requirements-pin.txt` 当前环境为基线；CI 加 step：用 pin 文件装环境跑 pytest |
...
|---|---|---|
| C1 | Chunk 体积硬阀门 | §3.5 |
| C2 | eval runtime 对齐 v2 chunk store | §3.5 |
| C3 | 统一模型调用 gateway（exact cache + 信号量 + 退避） | §3.6 |
| C4 | Rerank 高置信跳过 | §3.6.2 |
| C5 | Contextual summary 改离线 | §3.7 |
| C6 | Generation 证据打包预算 + per-paper 上限 | §3.8 |
| C7 | 429 / Retry-After 处理 | §3.6.1 步骤 5 |
