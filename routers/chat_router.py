...
async def chat_ask(req: ChatRequest) -> ChatResponse:
    ...
    try:
        url, headers, payload = _build_chat_request(req.query, req.context, req.llm, history=req.history)
    ...
    return ChatResponse(answer=answer, model=model_used, usage=usage)
...
async def chat_stream(req: ChatStreamRequest) -> StreamingResponse:
    ...
    """
    try:
        url, headers, payload = _build_chat_request(req.query, req.context, req.llm, stream=True, history=req.history)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
            ...
            async with httpx.AsyncClient(timeout=120.0) as client:
                async with client.stream("POST", url, json=payload, headers=headers) as resp:
                    if resp.status_code != 200:
                        ...
                        err_msg = _extract_provider_error_message(body.decode("utf-8", errors="replace")[:500])
                            ...
                            try:
                                ...
                                if provider_key == "claude":
                                    ...
                                    if chunk_type == "content_block_delta":
                                        ...
                                        if usage:
                                            yield f"data: {json.dumps({'event': 'usage', 'usage': usage, 'model': req.llm.model})}\n\n"
                                ...
                                usage = chunk.get("usage")
                                if usage:
                                    yield f"data: {json.dumps({'event': 'usage', 'usage': usage, 'model': chunk.get('model', req.llm.model)})}\n\n"
